...
- That about signs? your Jaccard approach seems to not use them. Not sure the Euclidean distance used by `pheatmap` (in @fig-adj-heatmap) is fundamentally better, but it does take sign into account.
:::

Many variations of what happens in the next code chunk are possible: e.g., one could do the matrix decompositions separately for the positive (up-regulated) and negative (down-regulated) neighbourhoods, rather than taking the absolute value of `abs(adj)`.
...
#|   - The `j`-th bicluster
gglem + geom_point(aes(colour = cell), size = 0.3) 
gglem + geom_point(colour = mycolours[as.character(bic_cells)], size = 0.3) 
```

TO DO:

- Why does @fig-bicluster-and-celltypes not show a clearer pattern? Is it better for other biclusters? Should we do it sign-specific (without `abs`) instead?
