...
#| fig-width: 5
#| fig-height: 5
ggplscr1 + geom_point(aes(colour = de), size = 0.2) +
...
To help identify the most interesting changes, `lemur` can aggregate the results of each gene and identify a set of cells (which we call _neighbourhood_) with the most prominent coherent pattern of differential gene expression for that gene. Subsequently, to conduct statistically valid differential expression inference, `lemur` uses the pseudobulk approach to compute a gene count for each replicate (here: patient) on the test data that were previously set aside. The output is a `data.frame` with one row for each gene. The `neighborhood` column is a list column, where each element is a vector with the IDs of the cells that are inside the neighbourhood. The other columns describe the significance of the expression change defined in the `test_de` call.
...
We can sort the table either by the `pval` or the `did_pval`. The `pval` measures the evidence of an expression change between control and stimulated condition for all cells inside the neighbourhood. In contrast, the `did_pval` measures how much larger the expression difference is for the cells inside the neighbourhood versus outside.
...
```{r, paged.print=FALSE}
#| label: find-neighbourhood-lemur-result-plots2
...
```{r}
#| label: neighbourhoods_to_long_dataframe-helper-function-1
#| echo: false
...
```{r}
#| label: neighbourhoods_to_long_dataframe-helper-function-2
...
```{r}
#| label: neighbourhoods_to_long_dataframe-helper-function-3
#| echo: false
...
## Bonus Step: Simultaneous clustering of genes and cells 

So far, we have identified cell neighbourhoods for individual genes.. While this is undoubtedly useful in many analyses, we are often interested in expression changes of entire gene programmes, i.e. larger sets of co-expressed genes. Investigating responses at the gene programme level can provide valuable insight into complex responses to changes in condition. `lemur` is particularly useful for this task since it provides us with a differential prediction on cell level as we saw in Step 2. Performing bi- or coclustering on this differential expression matrix allows us to identify neighbourhoods of cells that show coherent up- or downregulation of entire sets of genes, i.e. gene programmes.
