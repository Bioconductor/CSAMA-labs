...
A dedicated package for biclustering of `lemur` predictions is currently under development by the authors, and we aim to released it soon. The approach is based on a sparse matrix decomposition of the adjacency matrix (as in @fig-adj-heatmap) followed by an algorithm to assign cells and genes to biclusters, designed such that a cell or a gene can be part of _multiple_  biclusters. This is motivated by the biological fact that a gene can contribute to multiple biological processes, and cells can respond to condition changes through the expression of multiple different programmes. Biclusters can be rated based on reproducibility and coherence of the bicluster submatrix.
