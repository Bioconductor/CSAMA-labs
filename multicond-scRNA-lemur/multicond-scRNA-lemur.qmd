...
ggum + geom_point(aes(color = stim), size = 0.3)
ggum + geom_point(aes(color = cell), size = 0.3)   
```
...
#| fig-width: 10
#| fig-height: 5
ggum + geom_point(aes(color = stim), size = 0.3) + facet_wrap(vars(ind), ncol = 4)
```
:::
...
So far, we have identified cell neighborhoods for individual genes.. While this is undoubtedly useful in many analyses, we are often interested in expression changes of entire gene programmes, i.e. larger sets of co-expressed genes. Investigating responses at the gene programme level can provide valuable insight into complex responses to changes in condition. `lemur` is particularly useful for this task since it provides us with a differential prediction on cell level as we saw in Step 2. Performing bi- or coclustering on this differential expression matrix allows us to identify neighborhoods of cells that show coherent up- or downregulation of entire sets of genes, i.e. gene programmes.
...
The current version of `lemur` already allows for a manual approximation to this problem using the neighborhoods identified in the previous step. As a reference, we again take the top expressed gene and its associated cell neighborhood. Then, we compare this neighborhood to all other significant neighborhoods found. The similarity of these neighborhoods can be quantified using the Jaccard index $J(A, B)={|A \cap B|}/{|A \cup B|}$, which measures the similarity of two sets.
...
As we can see, many neighborhoods show strong overlap, and the mean expression indicates that we find both up- and downregulated genes. Setting a threshold of, say, $J > 0.9$, we obtain:
...
```{r}
#| label: fig-master-neighborhood
#| fig-cap: 
#|   - Master neighborhood from repeated intersection of gene neighborhoods of similar Jaccard index
...
p_master_neighborhood <- as_tibble(colData(sce), rownames = "cell_name") |>
  ...
  ggplot(aes(x = umap[, 1], y = umap[, 2])) +
  geom_point(aes(color = inside), size = 0.3) +
  scale_color_manual(values = c(`FALSE` = "grey80", `TRUE` = "red")) +
  coord_fixed() +
  labs(title = "Master neighborhood", color = "Inside")

p_master_neighborhood
```

We now have a list of `{r}length(master_result$genes)` genes showing significant differential expression within a neighborhood of `{r}length(master_result$neighborhood)` cells. To visualise the result, we can color all cells belonging to the master neighborhood as shown in @fig-master-neighborhood.


```{r, paged.print=FALSE}
#| label: fig-umap-bicluster-selected-genes
#| fig-cap: 
#|   - Differential expression of selected genes associated to the master neighborhood we constructed earlier.
...
We can visually confirm that the identified master neighborhood indeed shows coherent up- or downregulation across these genes. For example, taking the first four genes we identified and plotting their differential expression on the UMAP in @fig-umap-bicluster-selected-genes.

Note that gene expression need not be restricted to that neighborhood. Indeed, the intersection approach only guarantees to identify a neighborhood of coherent expression patterns -- any gene belonging to this “bicluster” may also show a very different expression pattern in other cell neighborhoods. This highlights once again the need to identify non-exclusive and non-exhaustive biclusters, since genes can participate in multiple programmes, and programmes themselves can differ across cell neighborhoods.

::: {.callout-note collapse="true"}
## Challenge: Quantifying the coherence of "biclusters"
The goal of identifying biclusters and neighborhoods of co-expression is not only to find genes that are, for example, significantly up- or downregulated within a neighborhood, but rather to identify genes that show a coherent expression pattern. For example, if there is a gradient of differential expression within that neighborhood, it would be interesting to see if other genes from the bicluster follow the same trend. In that sense, coherence is a stronger condition than simply averaged regulation over a given neighborhood.
