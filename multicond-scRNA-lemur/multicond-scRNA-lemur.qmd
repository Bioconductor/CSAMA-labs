...
## Finding neighbourhoods (Step 3) and test for differential expression (Step 4)
...
neighbourhoods_to_adjacency_matrix <- function(x, fit, out = c("matrix", "longdataframe")) {
  ...
  for (i in seq_along(gene_names))
    mat[nei[[i]], gene_names[i]] <- TRUE
  
  switch(out,
    matrix = mat,
    longdataframe = as_tibble(as.data.frame.table(mat, responseName = "inside", stringsAsFactors = FALSE))
  )
}
```
...
cells_inside_df <- neighbourhoods_to_adjacency_matrix(top_did_gene, fit, "longdataframe")
...
The `lemur` neighbourhoods are adaptive to the underlying expression patterns for each gene. This means that they provide optimal power to detect differential expression as well the set of cells for which the gene expression changes most.
...
So far, we have identified cell neighbourhoods for individual genes. While this is useful, the results for hundreds of genes can be overwhelming, and we may want to know whether certain sets of genes have quite similar neighbourhoods, i.e., are co-differentially expressed. 

TO DO: 

- use `neighbourhoods_to_adjacency_matrix` to explain the bipartite graph and the adjacency matrix.
- let's find a better name than master neighbourhood

```{r}
#| label: adjmat
adj <- neighbourhoods_to_adjacency_matrix(dplyr::filter(nei, adj_pval < 0.15), fit, "matrix")
dim(adj)
```
```{r}
...
#| fig-width: 10
#| fig-height: 6
#| fig-cap: "Adjacency matrix. Rows:cells, columns: genes"
library("pheatmap")
pheatmap(adj[sample(nrow(adj), 2500), ] + 0, 
  show_rownames = FALSE, show_colnames = FALSE, treeheight_row = 0, treeheight_col = 0)
```

Thus, we can do bi- or co-clustering on this differential expression matrix allows us to identify neighbourhoods of cells that show coherent up- or downregulation of entire sets of genes, i.e. gene programmes.

::: {.callout-warning collapse="false"}
## Definition
Biclustering or Coclustering is a data analysis technique that simultaneously groups both rows and columns of a matrix to identify subsets of features that exhibit coherent patterns within subsets of samples. Unlike traditional clustering, which clusters either rows or columns independently across all data, biclustering discovers local patterns by identifying groups of rows that behave similarly only across specific groups of columns. Thus, a bicluster is a submatrix whose rows and columns jointly satisfy a coherence criterion.
:::

A dedicated pipeline for biclustering of `lemur` predictions is currently in development and will be released in the near future. This approach is based on a sparse matrix decomposition of the lemur differential expression followed by a discrete recruitment strategy to assign cells and genes to _multiple_ biclusters at the same time and in an unsupervised fashion -- genes can be part of multiple response programmes, and cells can respond to condition changes through the expression of multiple different programmes. Biclusters can be rated based on reproducibility and coherence of the bicluster submatrix.

The current version of `lemur` already allows for a manual approximation to this problem using the neighbourhoods identified in the previous step. As a reference, we again take the top expressed gene and its associated cell neighbourhood. Then, we compare this neighbourhood to all other significant neighbourhoods found. The similarity of these neighbourhoods can be quantified using the Jaccard index $J(A, B)={|A \cap B|}/{|A \cup B|}$, which measures the similarity of two sets.
