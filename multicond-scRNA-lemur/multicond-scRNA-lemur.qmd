...
- Migrate all heatmaps to `ComplexHeatmap::Heatmap` and [seriation by TSP](https://nicolas.kruchten.com/content/2018/02/seriation/), as in @fig-morenmf?
...
<!-- Below, moving to newer heatmap package called 'ComplexHeatmap', and serialization via TSP -- maybe should use that above, too?
     TO DO: 
       - fix colors, make consistent with above. 
       - in the second plot, threshold the bicluster membership to 0/1, and use different colours for each bicluster: 
            does it look good?       
-->
...
#| message: false
k = 12
adjnmf <- RcppML::nmf(abs(adj), k = k, verbose = FALSE)
adjapp <- with(adjnmf, w[, 1:k, drop = FALSE] %*%  diag(d[1:k]) %*% h[1:k,, drop = FALSE])  
library("seriation")
adjs <- abs(adj[csub, ])
adjapps <- adjapp[csub, ] 
```
...
#| outwidth: "100%"
#| message: false
ord <- seriate(adjs, method = "BEA_TSP")
myhm <- function(x, ord)
  Heatmap(x, name = deparse(substitute(x)), #col = mycolours, 
    row_order = get_order(ord, 1), column_order = get_order(ord, 2),
