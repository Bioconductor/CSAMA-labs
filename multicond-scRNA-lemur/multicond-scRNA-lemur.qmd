...
- Above, you say "sparse matrix decomposition of the adjacency matrix", but the matrix does not seem to be very sparse, once you select genes with a "significant" neighbourhood. Should we not rather talk about _approximation with a low rank matrix_, rather than sparsity?]
...
- the repeated intersection approach seems overly stringent / conservative, i.e., it will loose many cells just because they didn't reach a thresholds for one particular gene. I'm hoping for something more like smoothing, literally, finding almost but not perfectly coloured rectangles in @fig-adj-heatmap
:::

Many variations of what happens in the next code chunk are possible: e.g., could do the matrix decompositions separately for the positive (up-regulated) and negative (down-regulated) neighbourhoods, rather than taking the absolute value of `abs(adj)`.
...
adjnmf <- RcppML::nmf(abs(adj), k = 3, verbose = FALSE)
adj_jn <- with(adjnmf, w[, j, drop = FALSE] %*% h[j,, drop = FALSE] * d[j])  # the matrix approximation from the j-th NMF component
```

::: {.callout-note collapse="true"}
## Plot `adj_jn`, i.e., the matrix spanned by the `j`-th NMF component `adjnmf$h[j, ]` and `adjnmf$w[, j])`, as a heatmap similar to @fig-adj-heatmap.
```{r}
#| label: fig-adj-jn
#| fig-width: 5
#| fig-height: 7
...
To order the rows and columns in this plots, we use the size of the NMF coefficients `w` and `h`. This is unlike in @fig-adj-heatmap, where we used, in lack of better alternatives, a dendrogram ordering.
We also need to do some extra index gymnastics to keep track of the random subsampling of `r length(cells_sample)` rows for plotting.
...
Let's pick a bicluster. The choice of the thresholds is motivated by the histograms above. One could automate this, e.g., using a two-component mixture model or $k$-means with $k=2$.

```{r}
#| label: pick-bicluster
threshold = c(h = 0.002, w = 0.00012)
bic_genes = adjnmf$h[j, ] > threshold["h"]
bic_cells = adjnmf$w[, j] > threshold["w"]  
table(bic_genes)
table(bic_cells)
cells_sample = sample(nrow(adj), 2048)
adj_sample_abs = abs(adj[cells_sample, ])
```


**TO DO** plot the heatmap ordered by inside first
```{r}
...
#| out-width: "75%"
#| fig-cap: "The matrix approximation from the j-th NMF component"
#pheatmap(adj_sample_abs[row_order, col_order], show_rownames = FALSE, show_colnames = FALSE, cluster_rows = FALSE, cluster_cols = FALSE,
#         annotation_row = data.frame(inside = bic_cells[csamp] + 0, row.names = rownames(adj)[csamp]),
#         annotation_col = data.frame(inside = bic_genes + 0, row.names = colnames(adj))
#)
```
