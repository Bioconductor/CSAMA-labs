...
want it, but nothing complicated. Note the random subsetting of rows by `csub`. This is because the `r nrow(adj)` rows of `adj` are too many to see on the screen anyway (your screen will likely not have more than a few thousand pixels in vertical direction), and the subsetting saves computation time.
```{r}
...
Looking at @fig-adj-heatmap, we immediately see that there is a lot of structure, and that bi- or coclustering of this matrix should be a fruitful approach to unravel that structure. That is, we are going to look for sets of genes who have similar neighbourhoods (cells). We may be able to interpret these as biological processes or regulatory programmes. Similarly, we can look for sets of cells that show differential co-expression. We can use this refine our notions of cell type defined from static expression maps, discover new subpopulations or commonalities.
...
- Many variations of what happens in the next code chunk are possible: 
  1. Whether to do the matrix decompositions separately for the positive (up-regulated) and negative (down-regulated) neighbourhoods, or take the absolute value `abs(adj)`, as we do below. 
  2. Choice between singular value decomposition (SVD; for which we below use the `irlba` method from the eponymous package; and where probably some additional sparsity-inducing regularisation is warranted) or non-negative matrix factorization (NMF). The positivity constraint seems relevant and necessary for adjacency matrices.
:::
