...
5. Bi-clustering of cells and genes with similar differential expression patterns

A basic workflow with might look like this^[Future versions might simplify this into a single end-to-end function.]:
```{r}
...
```
Let's look at a heatmap rendering of `adj`. We use the `Heatmap` function from the `ComplexHeatmap` package. Calling it needs a litany of parameters to customise the plot as we
want it, but nothing complicated. Note the random subsetting of rows by `csub`. This is because the `r nrow(adj)` rows of `adj` are too many to see on the screen anyway (your screen will likely only have a few thousand pixels in vertical direction), and the subsetting saves computation time.
```{r}
...
- What about using signs? The Jaccard approach below seems to not use them. In the current NMF I also take `abs(adj)`. But one could of course explore whether taking sign into account is not better.
...
To order the rows and columns in this plot, we use the size of the NMF coefficients `w` and `h`. This is unlike in @fig-adj-heatmap, where we used, in lack of better alternatives, a dendrogram ordering^[See, e.g., <https://jokergoo.github.io/ComplexHeatmap-reference/book/a-single-heatmap.html> Section 2.3.4.]. We also need to do some extra index gymnastics to keep track of the random subsampling of `r length(csub)` rows for plotting.
...
Now we are able to visualize this bicluster on top of a heatmap like in @fig-adj-heatmap. Note that while @fig-adj-heatmap distinguishes between -1 and +1 for indicating neighbourhoods in which the gene is down- or up-regulated, we have here chosen to do the NMF on the absolute values. 

Most of the code below is for ordering the rows and columns of the matrix using _seriation_, i.e., a one-dimensional embedding of multivariate objects. For this, we use a travelling salesperson algorithm as implemented in the [seriation package](https://github.com/mhahsler/seriation). The seriation is dominated by bicluster membership (the `LARGE` term in the code below), and then, by the finer structure within `mat` (the absolute value of the submsampled adjacency matrix) itself. The at the top and right sides of the matrix indicate membership in our bicluster (`j=``r j`).
...
mycolourramp <- colorRamp2(c(0, 1), c("#ffffff", "royalblue2"))
