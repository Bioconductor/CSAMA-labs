...
neighbourhoods_to_adjacency_matrix <- function(x, fit, out = c("matrix", "longdataframe")) {
  ...
  switch(out,
    matrix = mat, # signed integer matrix cells x genes
    longdataframe = as_tibble(as.data.frame.table(mat != 0, responseName = "inside", stringsAsFactors = FALSE))  # logical values
  )
}
```
...
So far, we have identified cell neighbourhoods for individual genes. While this is useful, the results for hundreds of genes can be overwhelming, and we may want to go step further and know whether certain sets of genes have quite similar neighbourhoods, i.e., are co-differentially expressed. In this step, which was not yet described by @AhlmannEltze:NatGen:2025, we use biclustering to address this question.
...
library("pheatmap")
pheatmap(adj[sample(nrow(adj), 1800), ] + 0,   # random subset to save compute time and since one cannot see more anyway (pixel resolution)
  show_rownames = FALSE, show_colnames = FALSE, treeheight_row = 0, treeheight_col = 0)
```

Looking at @fig-heatmap, we immediately see that there is a lot of structure, and that bi- or coclustering of this matrix should be a fruitful approach to unravel that structure. That is, we are going to look for sets of genes who have similar neighbourhoods (cells). We may be able to interpret as biological processes or regulatory programmes.

::: {.callout-warning collapse="false"}
## Biclustering
[Biclustering or coclustering](https://en.wikipedia.org/wiki/Biclustering) is a data analysis technique that simultaneously groups both rows and columns of a matrix to identify subsets of rows that exhibit coherent patterns within subsets of columns. 
:::
...
TODO: - let's find a better name than master neighbourhood
