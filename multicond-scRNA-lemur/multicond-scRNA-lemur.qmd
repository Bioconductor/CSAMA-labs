...
A dedicated pipeline for biclustering of `lemur` predictions is currently in development and will be released in the near future. This approach is based on a dense matrix decomposition of the lemur differential expression followed by a discrete recruitment strategy to assign cells and genes to _multiple_ biclusters at the same time and in an unsupervised fashion -- genes can be part of multiple response programmes, and cells can respond to condition changes through the expression of multiple different programmes. Biclusters are inferred directly from the predicted differential expression using different coherence measures of the identified submatrices.
...
We now have a list of `{r}length(master_result$genes)` genes showing significant differential expression within a neighborhood of `{r}length(master_result$neighborhood)` cells. To visualise the result, we can color all cells belonging to the master neighborhood. 
...
We can visually confirm that the identified master neighborhood indeed shows coherent up- or downregulation across these genes. For example, taking the first four genes we identified,

```{r, paged.print=FALSE}
#| label: umap-bicluster-selected-genes
