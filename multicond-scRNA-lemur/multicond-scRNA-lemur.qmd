...
library("circlize")
csub <- sample(nrow(adj), 2048) # random subset to save compute time and since we cannot see more anyway (pixel resolution)
Heatmap(adj[csub, ], name = "adj", col = colorRamp2(c(-1, 0, 1), c("RoyalBlue", "#f0f0f0", "firebrick1")),
...
- What about signs? your Jaccard approach seems to not use them. Not sure the Euclidean distance used by `Heatmap` (in @fig-adj-heatmap) is fundamentally better, but it does take sign into account.
...
adjnmf <- RcppML::nmf(abs(adj), k = 12, verbose = FALSE)
...
- Move the below challenge  Quantifying the coverage of the "biclusters" into the Appendix — and hopefully can serve towards future development of LEMUR2

::: {.callout-note collapse="true"}
## Challenge: other biclusters
Redo the above enrichment analysis for cell types and gene pathways for another cluster (another `j`).
:::
...
# Subsampling for visualisation -  there are no more pixels on the display anyway, and saves time.
csub        <- sample(nrow(adj), 2048)
adjs        <- abs(adj[csub, ])
