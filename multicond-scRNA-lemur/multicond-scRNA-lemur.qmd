...
#| label: svdnmf
#| message: false
# many variations of this possible: e.g. do separately for positive (up-regulated) and negative (down-regulated) neighbourhoods, rather than taking abs
set.seed(0xdada)
j = 1
adjsvd  <- irlba::irlba(abs(adj))
adj_1 <- with(adjsvd, u[, j, drop = FALSE] %*% t(v[, j, drop = FALSE]) * d[j])

adjnmf <- RcppML::nmf(abs(adj), k = 3)
adj_2 <- with(adjnmf, w[, j, drop = FALSE] %*% h[j,, drop = FALSE] * d[j])
```
```{r}
...
#| fig-height: 7
#| out-width: "75%"
#| fig-cap: "Bicluster constructed from the first column of the NMF of `adj`."
bic_genes = adjnmf$h[j, ] > 0.002 # could automate this using 2-component mixture model or k-means with k=2
bic_cells = adjnmf$w[, j] > 0.00012 
table(bic_genes)
table(bic_cells)
csamp = sample(nrow(adj), 2048)
pheatmap(abs(adj[csamp, ]), show_rownames = FALSE, show_colnames = FALSE, treeheight_row = 0, treeheight_col = 0,
