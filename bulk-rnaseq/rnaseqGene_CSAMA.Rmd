...
More information about how the raw data was processed is available in this [script](https://gist.github.com/mikelove/5a8134e57f652f970f1a176efc900cbe).
...
* The ranges are chosen based on the most abundant isoform (new since 2023)
...
`r Biocpkg("edgeR")` provides a convenient wrapper function to generate a *DGEList* from the gene-level *SummarizedExperiment* object:

```{r dgelist, message=FALSE}
library(edgeR)

dge <- edgeR::DGEListFromTximeta(sg, samples = colData(sg))
names(dge)
head(dge$samples)
```
...
Finally, we calculate TMM normalization factors.
